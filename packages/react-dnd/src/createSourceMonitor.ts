...
export interface ISourceMonitor extends IDragDropMonitor {
	canDrag(): boolean
	receiveHandlerId(sourceId: string): void
}

class SourceMonitor implements ISourceMonitor {
...
export default function createSourceMonitor(manager: IDragDropManager<any>) {
	return new SourceMonitor(manager) as ISourceMonitor
}
